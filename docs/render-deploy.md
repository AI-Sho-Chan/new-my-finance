...
render.yaml の初期値では VITE_BACKEND_URL を https://REPLACE_WITH_BACKEND_URL に設定しています。デプロイ完了後、Render のフロントエンドサービス画面から **Environment** を開き、実際のバックエンドの URL（例: https://myfinance-backend-osre.onrender.com）へ書き換え、**Save changed variables** → **Deploy changes** を実行してください。

必要に応じて **Manual Deploy → Clear build cache & deploy latest commit** を選び、キャッシュを無効化したビルドを行ってください。これにより VITE_BACKEND_URL が確実にビルドへ反映されます。
...
- フロントエンドサービスではビルド時に 
pm ci --include=dev を実行するため、Render の Node 18 環境で devDependencies もインストールされます。環境変数 NODE_VERSION を 18 に設定しているので、Blueprint から削除しないでください。
