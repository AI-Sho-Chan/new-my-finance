import { ok, send, UA } from './_utils.mjs';
...
export default async function handler(req, res) {
  ...
  try {
    const body = typeof req.body === 'object' && req.body != null ? req.body : (()=>{ try { return JSON.parse(req.body || '{}'); } catch { return {}; } })();
    ...
    for (const sym of symbols){
      try {
        ...
        if(uptrend){
          ...
          if(nearBand && rsiOk){
            signals.push({ type:'押', date: ts[iLast], strength: 0.6, reason: 'Pullback to EMA20 - 1.5*ATR with RSI zone' });
          }
        }
        out[sym] = { signals };
      } catch (e) {
